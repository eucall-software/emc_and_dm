...
Written by V. Elser; last modified on Tue Feb 10 12:07:25 SGT 2015.
...
double (*pix)[3] ;
int (*stop)[3] ;
int *ones, *multi, **place_ones, **place_multi, **count, * prob_q ;
double ***intens1, ***intens2, ***inter_weight, *s, *data_info ;
float **w_in, **w_out ;
...
void print_intens() ;
void free_mem() ;
void make_rot(int, double**, double**) ;
...
double maximize()
	...
	double  *u, *p ;
	u = malloc(num_rot * sizeof(*u)) ;
	p = malloc(num_rot * sizeof(*p)) ;
	...
	for (d = 0 ; d < num_data ; ++d)
		...
		max_exp = -100. * num_pix ;
		#pragma omp parallel shared(d, data_info, prob_q, w_in, count_d, num_rot, ones, multi, place_ones_d, place_multi_d, max_exp) private(r, w_in_r, t)
		{
		...
		#pragma omp parallel shared(d, u, data_info, p, num_rot, p_sum, mutual_info, quat, s, w_out, prob_q, ones, place_ones_d, multi, place_multi_d) private(r, w_out_r, t)
		{
		#pragma omp for schedule(dynamic, CHUNKSIZE) nowait
		for (r = 0 ; r < num_rot ; ++r)
			{
			...
			if (p[r] > 0.)
				data_info[d] += p[r] * log(p[r] / quat[r][4]) ;
...
void print_intens()
	...
	fp = fopen("mutual_info.dat", "w") ;
	for (t = 0 ; t < num_data ; ++t)
		fprintf(fp, "%21.15e ", data_info[t]) ;
	fclose(fp) ;

	fp = fopen("most_likely_orientations.dat", "w") ;
	for (t = 0 ; t < num_data ; ++t)
		fprintf(fp, "%d ", prob_q[t]) ;
	fclose(fp) ;
...
int setup()
	...
	rot = malloc(3 * sizeof(double*)) ;
	for (r = 0 ; r < 3 ; r++)
		rot[r] = malloc(3 * sizeof(double)) ;
		
	quat = malloc(num_rot * sizeof(double*)) ;
	for (r = 0 ; r < num_rot ; ++r)
		{
		quat[r] = malloc(5 * sizeof(double)) ;
		for (i = 0 ; i < 5 ; ++i)
	...
	prob_q = malloc(num_data * sizeof(int)) ;
	data_info = malloc(num_data * sizeof(double)) ;
...
void make_rot( int r, double **curr_quat, double** curr_rot )
	{
